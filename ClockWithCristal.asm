...
TA_CTL      .equ    TASSEL_1|ID_0|MC_1|TAIE ; SMCLK,/8,UP,IE
TA_FREQ     .equ    0x0001                 ; FRECUENCIA DEL timerA
;-------------------------------------------------------------------------------
...
SetupWDT    mov.w   #WDT_MDLY_8,&WDTCTL  ; WDT 1s*4 interval timer
            bis.b   #DIVA_0,&BCSCTL1        ; ACLK/2
...
;-------------------------------------------------------------------------------
	        ...
	        mov.w   #0h,r10                 ;para hacer el conteo	del segundo
	        mov.w   #0h,r11                 ;bandera para ver cual modificar seg, min horas
            mov.w   #0h,r12                 ;display a encender
            mov.w   #0h,r15                 ;cunta unsegundo para el pulsado del boton
;-------------------------------------------------------------------------------
; Event Loop
;-------------------------------------------------------------------------------
...
WDT_ISR:    mov.w   #0h,&P1OUT              ; Se limia la entrada del boton (muy impoetante)
            cmp.w   #80h,&P1IN              ;boton presionado?
              jz      tiempo                ;si, salta a tiempo para verifivar se es necesario cambiar a otra bandera
            cmp.w   #0,WDT_dcnt             ;No, SE PULSO ALGUNA TECLA PARA REALIZAR EL DEBOUNCING?
              jeq   WDT_02                  ; NO
            sub.w   #1,WDT_dcnt             ; SI, DECREMENTA EL CONTADOR, 0?
              jne   WDT_02                  ; NO
              ;-------ELIGE CUAL CAMBIAR SEGUNDOS, MINUTOS  O HORAS dependiendo de la bandera activa
            cmp.b    #0h,r11;
              jz     MoSeg
            cmp.b    #1h,r11;
              jz     MoMin
...
tiempo: inc.w    r15                  ; incrementa el regitro de conteo de presion boton
        cmp.w    #0x00C5,r15          ;Tiempo de presion Boton igual tiempo de cambio?
        jeq      Opcion               ;Si, salta a opcion donde se cambia la bandera
          reti
Opcion: inc.w    r11                  ; incrmenta la bandera
        cmp.w    #3h,r11 ;            ; bandera igual a 3?
        jeq      reiniciar            ;Si,reiniciar bandera
          reti
reiniciar:
          mov.w   #0h,r11
          reti
          ;-----------------Incrementa segundos, minutos o horas depediendo de lo elegido
MoSeg:
           mov.w    #0,r15
           inc.w    r4
           cmp.w    #10,r4
